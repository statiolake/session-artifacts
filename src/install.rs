...
Use `session-whiteboard close` to mark a session inactive while retaining its
board for resume. Use `session-whiteboard clean` only when the HTML and its
registry entry should be permanently deleted. Cleanup is explicit; there is no
reliable SessionEnd action that can ask the agent to rewrite the board.
...
`close` and `clean` commands; no SessionEnd hook is installed.\n\n\
